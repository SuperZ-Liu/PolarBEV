...
from polarbev.utils.geometry import (
...
from polarbev.utils.instance import convert_instance_mask_to_center_and_offset_label
from polarbev.utils.lyft_splits import TRAIN_LYFT_INDICES, VAL_LYFT_INDICES
...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def get_input_data(self, rec):
        ...
        for cam in cameras:
            ...
            lidar2img = viewpad @ lidar_to_sensor
            ...
            lidar2imgs.append(lidar2img.unsqueeze(0).unsqueeze(0))

        images, intrinsics, extrinsics, lidar2imgs = (torch.cat(images, dim=1),
                                          torch.cat(intrinsics, dim=1),
                                          torch.cat(extrinsics, dim=1),
                                          torch.cat(lidar2imgs, dim=1)
                                          )

        return images, intrinsics, extrinsics, lidar2imgs
    ...
    def __getitem__(self, index):
        ...
        """
        data = {}
        keys = ['image', 'intrinsics', 'extrinsics', 'lidar2imgs',
        ...
        for index_t in self.indices[index]:
            ...
            images, intrinsics, extrinsics, lidar2imgs = self.get_input_data(rec)
            ...
            data['lidar2imgs'].append(lidar2imgs)
...
def prepare_dataloaders(cfg, return_dataset=False):
    ...
    if cfg.DATASET.NAME == 'nuscenes':
        ...
        dataroot = os.path.join(cfg.DATASET.DATAROOT)
